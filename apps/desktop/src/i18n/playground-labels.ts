...
export const PLAYGROUND_LABELS: Record<AppLanguage, PlaygroundLabels> = {
  ...
  zh: {
    builtInToolDescriptions: {
      read: "读取本地文件内容，支持按行读取文本和查看图片。",
      write: "写入文件，自动创建所需目录；文件已存在时会覆盖原内容。",
      edit: "精确查找并替换文件中的文本，支持替换单处或所有匹配项。",
      apply_patch: "通过补丁新增、修改或删除文件，可一次处理多个文件。",
      ls: "列出目录中的文件和子目录。",
      tree: "以树状结构查看目录层级，可限制展开深度。",
      grep: "在文件内容中搜索文本或正则表达式，支持按文件类型筛选。",
      glob: "按文件名或路径模式查找文件，例如查找所有 TypeScript 文件。",
      bash: "执行终端命令并返回输出，支持设置工作目录和超时时间。",
      skill: "加载指定技能的说明，帮助模型按技能指引完成任务。",
      present_files: "将生成的本地文件展示为可打开或下载的附件。",
      web_fetch: "获取网页内容并转换为便于模型阅读的文本。",
      web_search: "搜索互联网，返回便于模型阅读的搜索结果。",
      weather_report: "查询指定地点今天的天气。",
      generate_image: "使用所选服务商和图片模型，根据文字描述生成图片，支持设置尺寸和水印。",
      list_voices: "列出运行端 Mac 已安装的语音、语言和示例文本，无需 API 密钥。",
      speak: "在运行端 Mac 上朗读文本，支持选择语音和语速；新的朗读会替换当前朗读。",
      stop_speaking: "停止此运行端启动的朗读，不影响其他应用的语音。",
      todo_write: "创建或更新任务清单，展示每项工作的待办、进行中或完成状态。",
      sleep: "等待指定时长后继续执行。",
      spawn_agent: "为独立任务创建子线程。当前运行会暂停，等待你点击创建子任务；子线程继承提示词、模型和所选工具，不继承对话历史，也不会自动运行。",
      ask_user_question: "向用户提问以澄清需求或获取选择，支持提供选项。",
      exec_code: "在可复用的会话中执行 Python、JavaScript 或 TypeScript，保留变量和中间结果，适合计算与数据分析。",
      calculator: "计算数学表达式，返回可验证的计算结果。",
      date_difference: "计算两个日期或时间之间的间隔，可选择是否包含起止时间；不涉及时区换算。",
    },
