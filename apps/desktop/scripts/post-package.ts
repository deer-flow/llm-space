...
// embed-windows-icon.ts), so the Setup exe created at the end of the build
// ships without an icon. This hook runs last (postPackage, after artifacts
// are refreshed) and embeds the icon into the Setup exe, re-creates the
// distribution zip with the same layout electrobun used, and refreshes the
// artifact copy. Remove together with embed-windows-icon.ts when upstream
...
if (!existsSync(iconSource)) {
  console.warn("[post-package] icon source missing, skipping:", iconSource);
  process.exit(0);
}
...
for (const channelDir of readdirSync(buildDir)) {
  ...
  console.info(`[post-package] embedded icon into ${setupPath}`);
  ...
  rmSync(stagingDir, { recursive: true, force: true });
  console.info(`[post-package] re-zipped ${zipPath}`);
  ...
  if (artifactDir && prefix) {
    ...
    console.info(`[post-package] refreshed artifact ${dest}`);
  }
}
