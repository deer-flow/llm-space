...
The app version has a **single source of truth: `apps/desktop/package.json`** — `electrobun.config.ts` imports it, and release CI fails if the pushed tag doesn't match. Cut releases with `bun run release` (stable) or `bun run release:canary`; the script (`scripts/release.ts`) runs `commit-and-tag-version` (conventional-commits-driven version bump + commit + `v*` tag, config in `.versionrc.json`) and pushes atomically. Changelog generation is off (`skip.changelog`), and CI publishes versioned releases with static notes (not `--generate-notes`), keeping release notes minimal. The tag triggers `.github/workflows/release.yml`: build → codesign/notarize (canary/stable only; needs the `ELECTROBUN_*` secrets) → smoke test → upload. Artifacts land in two GitHub releases: the rolling `updates` release is the machine-readable update feed (`release.baseUrl` points at it; **never delete its `.patch` files** — old installs chain through them), and a versioned release carries the DMG (macOS) and the NSIS installer exe (Windows) for humans. In-app auto-update lives in `bun/updates/` (background check → silent download → "restart to update" toast via the `updateStatusChanged` message); the dev channel never updates.

Releases ship for **macOS arm64 + x64** and **Windows x64** (Windows 11+ officially; Win10 is best-effort, and ARM64 Windows runs the x64 build via emulation). Intel macOS builds need `apps/desktop/scripts/fix-x64-headerpad.ts` (wired as the `postBuild`/`postWrap` hooks): electrobun's darwin-x64 core binaries have no Mach-O headerpad, so signing them corrupts `__text` and the app segfaults on launch (electrobun#485). The hook frees 16 bytes of load-command room before signing and no-ops everywhere else; delete it when upstream fixes #485.

### Windows port specifics

- **Platform gating**: `src/shared/platform.ts` works in both runtime contexts (navigator in the renderer, `process.platform` in bun) and owns the platform labels (`REVEAL_LABEL`, `MOVE_TO_TRASH_LABEL`, `MOD_KEY_LABEL`).
- **Window chrome**: frameless (`hiddenInset` ≈ `hidden` on Windows) with self-drawn caption buttons (`components/window-controls.tsx` → `windowMinimize`/`windowToggleMaximize`/`windowClose` commands). `trafficLightOffset` is macOS-only.
- **Menu/shortcuts**: the native `ApplicationMenu` is installed on macOS only; Windows/Linux shortcuts live in `lib/use-app-keymap.ts` (renderer keymap → command layer; F11 = fullscreen). Keep the two in sync when adding accelerators.
- **Page zoom**: Electrobun's `setPageZoom` is a no-op on Windows; zoom there flows bun → `applyPageZoom` message → `lib/use-css-zoom.ts` (CSS zoom), persisted via the same `window.json`.
- **Data root**: `%APPDATA%\llm-space` on Windows (`~/.llm-space` elsewhere); `LLM_SPACE_HOME` still overrides.
- **Built-in agent tools**: the `bash` tool resolves Git Bash and falls back to PowerShell (rewriting its own tool description — see `bun/tools/built-in/shell.ts`; `LLM_SPACE_BASH_PATH` overrides); the `grep` tool uses PATH `rg` or the copy CI bundles into `Resources/app/resources/rg.exe`.
- **Installer**: the user-facing Windows download is a thin NSIS wrapper (`apps/desktop/installer/llm-space.nsi`, compiled by `scripts/build-win-installer.ts`) that runs electrobun's console self-extractor hidden as the install engine and adds shortcuts, an Add/Remove Programs entry, and an uninstaller. The extractor stays the engine because the in-app updater **hardcodes** the layout `%LOCALAPPDATA%\tech.deerflow.llm-space\<channel>\app` (+ `self-extraction\<hash>.tar` for delta updates) — never install anywhere else. `scripts/brand-win-binaries.ts` (postBuild hook, pre-tar so the bytes are consistent across installer/feed/patches) rebrands every bundled exe's icon/VERSIONINFO — bun.exe owns the window and socket, so the taskbar icon and firewall prompt show *its* resources — and embeds the DPI manifest (`installer/win-app.manifest`; deliberately system-aware, not PerMonitorV2 — electrobun can't rescale webviews mid-flight; see the manifest header). `scripts/smoke-win-installer.ps1` is the CI acceptance gate (silent install/uninstall).
- **CI**: `release.yml` has a `windows-latest` matrix entry (unsigned for now — SignPath OSS signing planned for stable); `windows-branch-build.yml` builds an installable canary from the port branch for manual testing (`docs/windows-manual-test.md`).
