...
- **Landing-page release version:** the version and matching download URLs used by the GitHub Pages fallback are hard-coded in `apps/web/src/landing/lib/releases.ts` under `FALLBACK_RELEASES.stable`. This value is not updated automatically by the desktop release workflow. After each new version has built and published successfully, update `FALLBACK_RELEASES.stable.version` and both architecture-specific download URLs/sizes to that released version, then trigger the GitHub Pages build and deployment by merging/pushing the change to `main` or manually running `.github/workflows/pages.yml`. The browser normally refreshes from the GitHub Releases API at runtime, but the committed fallback must still match the latest successful stable release for offline, rate-limited, and failed-fetch cases.
